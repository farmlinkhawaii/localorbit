...
class core_model_products extends core_model_base_products
{
	...
	function get_catalog($domain_id=null,$org_id=-1)
	{
		...
		$sql = '
			...
			and (select sum(qty) from product_inventory where product_inventory.prod_id=p.prod_id and (expires_on > now() or expires_on is null) and (good_from <= now() or good_from is null)) > 0
