...
if($core->config['domain']['domain_id'] > 1)
{
	...
	if ($core->config['domain']['social_option_id'] == 1 && !empty($core->config['domain']['facebook'])) {
		...
		tweets.loadTweets(function() { $("#tweets").fadeIn();  console.log("tweets loaded");
			$("#tweets > span").append(\'<iframe allowtransparency="true" frameborder="0" scrolling="no" src="//platform.twitter.com/widgets/follow_button.html?show_screen_name=false&show_count=false&screen_name='.$core->config['domain']['twitter'].'" style="width:300px; height:20px;"></iframe>\');
		});');
	}
?>