...
class WPBakeryShortCode_VC_Message extends WPBakeryShortCode {
    ...
    protected function content( $atts, $content = null ) {
        ...
        $output .= '<div class="alert wpb_content_element '.$color.'"><div class="messagebox_text">'.wpb_js_remove_wpautop($content).'<button class="close" data-dismiss="alert" type="button">×</button></div></div>'.$this->endBlockComment('alert box')."\n";
...
class WPBakeryShortCode_VC_Toggle extends WPBakeryShortCode {
    ...
    protected function content( $atts, $content = null ) {
        ...
        $output .= apply_filters('wpb_toggle_heading', '<h3 class="wpb_toggle'.$open.'">'.$title.'</h3>', array('title'=>$title, 'open'=>$open));
