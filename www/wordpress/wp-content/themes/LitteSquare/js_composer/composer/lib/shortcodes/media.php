...
class WPBakeryShortCode_VC_Gmaps extends WPBakeryShortCode {
    ...
    protected function content( $atts, $content = null ) {
        ...
        $output .= "\n\t\t".'<div class="wpb_wrapper">';
        $output .= ($title != '' ) ? "\n\t\t\t".'<h3 class="wpb_heading wpb_map_heading">'.$title.'</h3>' : '';
        //$output .= wpb_widget_title(array('title' => $title, 'extraclass' => 'wpb_map_heading'));
        $output .= '<div class="wpb_map_wraper"><iframe width="100%" height="'.$size.'" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="'.$link.'&amp;t='.$type.'&amp;z='.$zoom.'&amp;output=embed"></iframe><img class="shadow" alt="" src="'.THEMESTUDIO_IMG.'/shadow-12.png"></div>';
