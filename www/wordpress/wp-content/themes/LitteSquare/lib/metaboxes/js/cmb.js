...
jQuery(document).ready(function ($) {
	...
	$('.cmb_upload_image').live('click', function () {
		var buttonLabel;
		formfield = $(this).prev('input').attr('name');
		buttonLabel = 'Use as ' + $('label[for=' + formfield + ']').text();
	...
	$('.cmb_upload_slider').live('click', function () {
		var buttonLabel;
	...
	$('.cmb_remove_slider_button').live('click', function () {
		$(this).parent().remove();
		return false;
	});
    ...
    window.send_to_editor = function (html) {
		...
		if (formfield) {
			...
			if (itemurl.match(image)) {
				if ( formslider ) {
					uploadStatus = '<div class="img_status"><input type="hidden" name="' + formfield + '[]" value="' + itemurl + '" /><img src="' + itemurl + '" alt="" /><a href="#" class="cmb_remove_slider_button">Remove Image</a></div>';
				}
				else {
			...
			if ( formslider ) {
				$('#' + formfield).siblings('.cmb_upload_status').slideDown().append(uploadStatus);
			}
			else
			{
