...
define( 'CMB_META_BOX_URL', THEMESTUDIO_URL . '/metaboxes/' );
...
// slides ajax filter
	...
	function show() {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( 'image' == $field['type'] && !isset( $field['allow'] ) ) $field['allow'] = array( 'url', 'attachment' );
			if ( 'image' == $field['type'] && !isset( $field['save_id'] ) )  $field['save_id']  = false;
			...
			$tr_style = !empty($field['tr_style']) ? ' style="'.$field['tr_style'].'"' : '';

			echo '<tr id="tr'.$field['id'].'"'.$tr_style.'>';
			...
			switch ( $field['type'] ) {
				...
				case 'text_medium':
					...
					break;
				case 'text_large':
					echo '<input class="cmb_text_large" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />','<p class="cmb_metabox_description">', $field['desc'], '</p>';
					break;
				case 'text_date':
				...
				case 'select':
					...
					foreach ($field['options'] as $option) {
						echo '<option value="', $option['value'], '"', ('' !== $meta ? $meta : $field['std']) == $option['value'] ? ' selected="selected"' : '', '>', $option['name'], '</option>';
					}
					echo '</select>';
				...
				case 'select_sidebar':
					...
					$sidebar = $theme_options['sidebar'];
					$default_sidebar = array("Page Sidebar","Contact Sidebar","Single Portfolio","Blog Sidebar","Blog Sidebar 2","Single Post Sidebar","Home Sidebar","Portfolio Sidebar 1","Portfolio Sidebar 2");
					//$sidebar = 
					$sidebar = array_merge($sidebar,$default_sidebar);
					sort($sidebar);
					/*echo '<select name="', $field['id'], '" id="', $field['id'], '">';
					echo '<option value="Page Sidebar"', ('' !== $meta ? $meta : $field['std']) == $val ? ' selected="selected"' : '', '>Page Sidebar</option>';
					echo '<option value="Contact Sidebar"', ('' !== $meta ? $meta : $field['std']) == $val ? ' selected="selected"' : '', '>Contact Sidebar</option>';
					echo '<option value="Single Portfolio"', ('' !== $meta ? $meta : $field['std']) == $val ? ' selected="selected"' : '', '>Single Portfolio</option>';
					echo '<option value="Blog Sidebar"', ('' !== $meta ? $meta : $field['std']) == $val ? ' selected="selected"' : '', '>Blog Sidebar</option>';
					echo '<option value="Blog Sidebar 2"', ('' !== $meta ? $meta : $field['std']) == $val ? ' selected="selected"' : '', '>Blog Sidebar 2</option>';
					echo '<option value="Single Post Sidebar"', ('' !== $meta ? $meta : $field['std']) == $val ? ' selected="selected"' : '', '>Single Post Sidebar</option>';
					echo '<option value="Home Sidebar"', ('' !== $meta ? $meta : $field['std']) == $val ? ' selected="selected"' : '', '>Home Sidebar</option>';
					echo '<option value="Portfolio Sidebar 1"', ('' !== $meta ? $meta : $field['std']) == $val ? ' selected="selected"' : '', '>Portfolio Sidebar 1</option>';
					echo '<option value="Portfolio Sidebar 2"', ('' !== $meta ? $meta : $field['std']) == $val ? ' selected="selected"' : '', '>Portfolio Sidebar 2</option>';
					*/
					echo '<select name="', $field['id'], '" id="', $field['id'], '">';
					foreach ($sidebar as $key => $val) {
						echo '<option value="', $val, '"', ('' !== $meta ? $meta : $field['std']) == $val ? ' selected="selected"' : '', '>', $val, '</option>';
					}
					echo '</select>';
					echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
					break;
				...
				case 'image':
					...
					if ( 'url' == $field['allow'] || ( is_array( $field['allow'] ) && in_array( 'url', $field['allow'] ) ) )
						$input_type_url="text";
					echo '<input class="cmb_upload_file" type="' . $input_type_url . '" size="45" id="', $field['id'], '" name="', $field['id'], '" value="', $meta, '" />';
					echo '<input class="cmb_upload_image button" type="button" value="Upload File" />';
					echo '<input class="cmb_upload_file_id" type="hidden" id="', $field['id'], '_id" name="', $field['id'], '_id" value="', get_post_meta( $post->ID, $field['id'] . "_id",true), '" />';					
					echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
					echo '<div id="', $field['id'], '_status" class="cmb_upload_status">';	
						if ( $meta != '' ) { 
				...
				case 'slider':
					$input_type_url = "hidden";
						$input_type_url="text";
						$this->show_field_slider($field,$meta);
				...
				case 'selectslider' :
					$input_type_url = "hidden";
						$input_type_url="text";
						$this->show_field_listslide2($field,$meta);
						//echo '<a href="#" class="cmb_remove_file_button" rel="', $field['id'], '">Remove Image</a>';
				...
				case 'selectgallery' :
					$input_type_url = "hidden";
						$input_type_url="text";
						$this->show_field_listgallery($field,$meta);
						//echo '<a href="#" class="cmb_remove_file_button" rel="', $field['id'], '">Remove Image</a>';
				...
				case 'selectportfolio' :
					$input_type_url = "hidden";
						$input_type_url="text";
						$this->show_field_listportflio($field,$meta);
						//echo '<a href="#" class="cmb_remove_file_button" rel="', $field['id'], '">Remove Image</a>';
				...
				case 'select_portfolio_category' :
					$input_type_url = "hidden";
						$input_type_url="text";
						$this->show_field_listportflio_category($field,$meta);
						//echo '<a href="#" class="cmb_remove_file_button" rel="', $field['id'], '">Remove Image</a>';
	...
	// List Slider
	function show_field_listslide2($field,$meta){
			echo "<select class='ts-select' name='".$field['id']."'  id='".$field['id']."' >";
			echo "<option value='no_slider'>". __('None','themestudio') ."</option>";
				...
				foreach ( $slides as $slide) {
					echo '<option value="', $slide->ID, '"', ('' !== $meta ? $meta : $field['std']) == $slide->ID ? ' selected="selected"' : '', '>', $slide->post_title, '</option>';
				};
				
			echo "</select><br />";
	}
	// List Gallery
	function show_field_listgallery($field,$meta){
			echo "<select class='ts-select' name='".$field['id']."'  id='".$field['id']."' >";
			echo "<option value='All'>". __('All','themestudio') ."</option>";
	...
	// List Portfolio
	function show_field_listportflio($field,$meta){
			echo "<select class='ts-select' name='".$field['id']."'  id='".$field['id']."' >";
			echo "<option value='no_slider'>". __('None','themestudio') ."</option>";
	...
	function show_field_listportflio_category($field,$meta){
			echo "<select class='ts-select' name='".$field['id']."'  id='".$field['id']."' >";
			echo "<option value='All'>". __('None','themestudio') ."</option>";
					
				//get posts
				$args = array( 'post_type' => 'portfolio', 'posts_per_page' => '-1' );
				$slides = get_terms('portfolio-category');
				//tt_debug($slides);
				//show select
				foreach ( $slides as $slide) {
					echo '<option value="', $slide->slug, '"', ('' !== $meta ? $meta : $field['std']) == $slide->slug ? ' selected="selected"' : '', '>', $slide->name, '</option>';
				};
	...
	function show_field_slider($field, $meta) {
		...
		echo '</ul>',
		'<a href="#" class="ts-ms-add-new">Add New Slide</a>';
	}
	function add_slide() {
	...
	function get_meta_slider( $id, $slide = array(), $i ) {
		...
		$return .= '<li id="'. $image .'_0'. $i .'" class="ts-ms-container">';
		
		$return .= '<div class="ts-ms-head">'. $title .'<a href="#" class="ts-ms-toggle">expand/close</a></div>';
		
		$return .= '<div class="ts-ms-body">';
			
		$return .= '<p>
					<label for="'. $id .'-'. $i .'-title">'. __('Slide title *', 'ThemeStudio') .'</label>
					<input type="text" class="ts-ms-input" id="'. $id .'-'. $i .'-title" name="'. $id .'['.$i.'][title]" value="'. $title .'" />
				</p>';
			
		$return .= '<p>
					<label for="'. $id .'-'. $i .'-title">'. __('Image', 'ThemeStudio') .'<br /><small>'.__('MUST be uploaded to your WordPress install for security reasons', 'ThemeStudio') .'</small></label>
					<input type="text" class="ts-ms-input" id="'. $id .'-'. $i .'-image" name="'. $id.'['.$i.'][image]" value="'. $image .'" />
					<input type="button" class="ms-upload-button button" id="'. $id .' "value="'. __('Upload', 'ThemeStudio') .'" />
				</p>';
					
		$return .= '<p>
					<label for="'. $id .'-'. $i .'-link">'. __('Link:', 'ThemeStudio') .'</label>
					<input type="text" class="ts-ms-input" id="'. $id .'-'. $i .'-link" name="'. $id.'['.$i.'][link]" value="'. $link .'" />
				</p>';
				
		$return .= '<p>
					<label for="'. $id .'-'. $i .'-lightbox">'. __('Lightbox Link:', 'ThemeStudio') .'<br /><small>'. __('this will override the link field above.', 'ThemeStudio') .'</small></label>
					<input type="text" class="ts-ms-input" id="'. $id .'-'. $i .'-lightbox" name="'. $id.'['.$i.'][lightbox]" value="'. $lightbox .'" />
				</p>';
				
		$return .= '<p>
					<label for="'. $id .'-'. $i .'-desc">'. __('Description (Optional) ', 'ThemeStudio') .'</label>
					<textarea class="ts-ms-textarea" rows="6" cols="20" id="'. $id .'-'. $i .'-desc" name="'. $id.'['.$i.'][desc]">'. $desc .'</textarea>
				</p>';
				
		$return .= '<p>
					<label for="'. $id .'-'. $i .'-video">'. __('Video URL:', 'ThemeStudio') .'<br /><small>'. __('Enter in a video URL that is compatible with WordPress\'s built-in oEmbed feature.', 'ThemeStudio') .' <a href="http://codex.wordpress.org/Embeds" target="_blank">'. __('Learn More', 'ThemeStudio') .'</small></a></label>
					<input type="text" class="ts-ms-input" id="'. $id .'-'. $i .'-video" name="'. $id.'['.$i.'][video]" value="'. $video .'" />
				</p>';
			
		$return .= '<a href="#" class="ts-ms-delete">Delete</a>
			</div>
...
function cmb_scripts( $hook ) {
  	if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
		...
		wp_register_script( 'cmb-scripts', CMB_META_BOX_URL . 'js/cmb.js', array( 'jquery', 'jquery-ui-core', 'media-upload', 'thickbox', 'farbtastic' ) );
		wp_register_script( 'cmb-custom', CMB_META_BOX_URL . 'js/custom.admin.js' );
		wp_enqueue_script( 'cmb-timepicker' );

		wp_enqueue_script( 'cmb-scripts' );
		wp_enqueue_script( 'cmb-custom' );
		wp_register_style( 'cmb-styles', CMB_META_BOX_URL . 'style.css', array( 'thickbox', 'farbtastic' ) );
		wp_enqueue_style( 'cmb-styles' );
		wp_register_style( 'cmb-metabox', CMB_META_BOX_URL . 'metabox.css');
		wp_enqueue_style( 'cmb-metabox' );
		
		wp_register_script( 'cmb-metaboxjs', CMB_META_BOX_URL . 'metabox.js' );
		wp_enqueue_script( 'cmb-metaboxjs' );
  	}
}
